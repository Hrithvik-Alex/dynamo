...
//! ZMQ KV-event republisher for external ("on-ramp") mode.
//!
//! In external mode the EPP fronts raw `vllm serve` pods that publish their
...
//! constructor that drives it lands in a subsequent change.
...
/// actually evicts the block). This reimplements — for the external EPP crate —
