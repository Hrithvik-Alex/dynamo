...
//! Kubernetes pod reflector for external ("on-ramp") mode.
//!
...
//! port *named* `http`), external mode fronts **raw `vllm serve` pods** that:
//!
...
//!
//! NOTE: this module is not yet wired into [`crate::epp::Router`]; the external
//! constructor that consumes it lands in a subsequent change.
...
pub struct ExternalPodReflector {
    store: Store<Pod>,
    ready: Arc<AtomicBool>,
    target_port: u16,
}

impl ExternalPodReflector {
    ...
    pub async fn spawn(k8s_namespace: &str, selector: &str, target_port: u16) -> Result<Self> {
        ...
        tracing::info!(
            ...
            selector = selector,
            target_port,
            "Starting external pod reflector for raw vLLM endpoint resolution"
        );
        ...
        tokio::spawn(async move {
            ...
            tracing::warn!("External pod reflector stream ended unexpectedly");
        });
        ...
        match tokio::time::timeout(INITIAL_SYNC_TIMEOUT, store_for_wait.wait_until_ready()).await {
            Ok(Ok(())) => {
                ...
                tracing::info!("External pod reflector initial LIST sync complete");
            }
            Ok(Err(e)) => {
                tracing::warn!(
                    error = %e,
                    "External pod reflector writer dropped before initial LIST; returning 503 until ready"
                );
            }
            Err(_) => {
                tracing::warn!(
                    "External pod reflector initial LIST timed out after {}s; returning 503 until ready",
                ...
                tokio::spawn(async move {
                    match store_for_background_wait.wait_until_ready().await {
                        Ok(()) => {
                            ...
                            tracing::info!(
                                "External pod reflector became ready after startup timeout"
                            );
                        }
                        Err(e) => {
                            tracing::error!(
                                error = %e,
                                "External pod reflector writer dropped while waiting; store stays not-ready"
                            );
                        }
                    }
                });
